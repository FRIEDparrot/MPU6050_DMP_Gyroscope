...
#elif defined MOTION_DRIVER_TARGET_STM32F10X

#include "Delay.h"
#include "I2C_func_general.h"
// if not use const, it will convert to uint8_t automatically (const prevent convert)
unsigned char i2c_write(unsigned char slave_addr, unsigned char reg_addr, unsigned char length, unsigned char *_data_){
    I2C_GEN_Write(slave_addr, reg_addr, length, _data_);
    return 0;
}

unsigned char i2c_read(unsigned char slave_addr, unsigned char reg_addr, unsigned char length, unsigned char *data_){
    I2C_GEN_Read(slave_addr, reg_addr, length, data_);
    return 0;
}
...
const struct hw_s hw = {
    0xD0,	 //addr -> note you should fixed this address
    1024,	 //max_fifo
    118,	 //num_reg
    340,	 //temp_sens
    -521,	 //temp_offset
...
const struct test_s test = {
    32768/250,		 // gyro_sens -> 250 degree 
    32768/16,		 //	accel_sens ->16g acceleration 
    0,				 //	reg_rate_div
    1,				 //	reg_lpf
    0,				 //	reg_gyro_fsr
    0x18,			 //	reg_accel_fsr
 ...
 *  @return     0 if successful.
 */
int mpu_init(void)  // change it to void 
{
...
#define q30  1073741824.0f         // the left number long to float
...
uint8_t mpu_dmp_init(void){
    uint8_t res = 0;
    I2C_GEN_InitFunction();  // init I2C port
    mpu_init();           // init MPU;

    res=mpu_set_sensors(INV_XYZ_GYRO|INV_XYZ_ACCEL);  // set the sensor needed 
    if(res)return 1;
    ...
    if(res)return 6; 
    res=dmp_set_fifo_rate(DEFAULT_MPU_HZ); // 200 
    if(res)return 7;   
    // res=run_self_test();		//self test 
    // if(res)return 8;
    res=mpu_set_dmp_state(1);	//enable dmp state
    if(res)return 9;
